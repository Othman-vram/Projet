...
% Page de garde INSA
\PageDeGarde{CHB_logo}{Création et Mise en Œuvre d'un Outil de Génération d'Images d'Anatomopathologie de Haute Définition à partir de Lames Scannées}{Stage de Spécialité - 4ème année}{Stagiaire : EL IDRISSI Othman\\Tuteur Entreprise : Sébastien HAPDEY (Physicien Médical)\\Enseignant Référent : Benoît GAUZÈRE}{2 juin 2025 - 31 août 2025\\Centre Henri Becquerel, Rouen}
...
\subsubsection{Technologies Analysées}

\paragraph{PyThostitcher}

\begin{tcolorbox}[colback=TechBlue!10, colframe=TechBlue, title=PyThostitcher - Outil Python de Suture SIFT]
...
\paragraph{HistoStitcher}

\begin{tcolorbox}[colback=WarningOrange!10, colframe=WarningOrange, title=HistoStitcher - Solution MATLAB Historique]
...
\begin{tcolorbox}[colback=SuccessGreen!10, colframe=SuccessGreen, title=ASHLAR - Solution Harvard Medical School]
...
\paragraph{FIJI/ImageJ}

\begin{tcolorbox}[colback=InfoGray!10, colframe=InfoGray, title=FIJI - Distribution ImageJ Enrichie]
...
\paragraph{Plugin Napari}

\begin{tcolorbox}[colback=TechBlue!10, colframe=TechBlue, title=Napari - Plateforme de Visualisation Moderne]
...
\subsubsection{Justification de la Solution Retenue}
...
\begin{center}
Vue du manque d'outils répondant à nos besoins spécifiques, nous avons choisi de développer un outil personnalisé \textit{from scratch}. Cette approche présente des avantages décisifs :
...
\begin{itemize}
    ...
    \item Adaptabilité complète aux besoins du Centre Henri Becquerel
\end{itemize}
\end{center}
\vspace{1em}
